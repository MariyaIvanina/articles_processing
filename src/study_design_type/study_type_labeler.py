from bert_models.base_bert_model_meta_model import BaseBertModelMetaModel
...
class StudyTypeLabeler(BaseBertModelMetaModel):

    def __init__(self, output_dir, label_list = list(StudyTypeLabels.STUDY_TYPE_LABEL_TO_NUMBER.values()), boost_model = "",
            ...
            label_column = "label", use_concat_results = False, meta_model_folder = "", use_one_layer = True, keep_prob=0.9,
            epochs_num = 1, multilabel=False):
        BaseBertModelMetaModel.__init__(self, output_dir, label_list, gpu_device_num_hub=gpu_device_num_hub, 
            boost_model = boost_model, gpu_device_num = gpu_device_num, batch_size = batch_size, max_seq_length = max_seq_length,
            bert_model_hub = bert_model_hub, model_folder = model_folder, label_column = label_column, use_concat_results=use_concat_results,
            meta_model_folder = meta_model_folder,  use_one_layer = use_one_layer, keep_prob=keep_prob,
            epochs_num = epochs_num, multilabel=multilabel)
    ...
    def get_prepared_df(self, temp_df, without_labels = False, label_column = "extended_study_type"):
        df = pd.DataFrame({
                        'title':temp_df["title"].values,
                        'abstract':temp_df["abstract"].values,
                        'text':temp_df["title"].values + "." +  temp_df["abstract"].values,
                        'label':[(StudyTypeLabels.STUDY_TYPE_LABEL_TO_NUMBER[val] if val in StudyTypeLabels.STUDY_TYPE_LABEL_TO_NUMBER else -1) for val in temp_df[label_column].values] if not without_labels else [-1]*len(temp_df)})
    ...
    def save_file_with_predictions(self, df, filename, withBoost = False, use_tail = False):
        res_prob, res_label, res_y = self.predict_for_df(df) if not withBoost else self.predict_with_meta_model(df, with_head_tail = use_tail, recreate_model = True)
