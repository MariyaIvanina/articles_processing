...
class OutcomesSentenceLabeler(BaseBertModel):
    ...
    def prepare_df_data_sentences(self, outcomes_train_label_data, label_to_train,
            include_empty_sentences=True, proportion_to_check=-1):
        train_data = {}
        real_sentence = {}
        for t in outcomes_train_label_data:
            text = t[0].strip().lower()
            real_sentence[text] = t[0]
            if include_empty_sentences or len(t[1]):
                if text not in train_data:
                    train_data[text] = 0
                if train_data[text] != 1:
                    train_data[text] = int(label_to_train in t[1])
